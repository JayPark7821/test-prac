...
public class UserControllerTest {
	...
	void 사용자는_인증_코드가_일치하지_않을_경우_권한_없음_에러를_내려준다() throws Exception {
		...
		assertThatThrownBy(() -> testContainer.userController
			.verifyEmail(1, "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaadddaab"))
	...
	void 사용자는_내_정보를_불러올_때_개인정보인_주소도_갖고_올_수_있다() throws Exception {
		...
		assertThat(result.getBody().getNickname()).isEqualTo("kok202");
		assertThat(result.getBody().getLastLoginAt()).isEqualTo(11111234);
		assertThat(result.getBody().getStatus()).isEqualTo(UserStatus.ACTIVE);
	...
	void 사용자는_내_정보를_수정할_수_있다() throws Exception {
		...
		// when
		ResponseEntity<MyProfileResponse> result = testContainer.userController
			.updateMyInfo("kok202@naver.com", UserUpdate.builder().nickname("test").address("Busan").build());
